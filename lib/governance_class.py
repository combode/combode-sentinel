...
class GovernanceClass(object):
    ...
    def vote(self, comboded, signal, outcome):
        return self.go.vote(comboded, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, comboded):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(comboded, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(comboded, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import combodelib
        obj_data = combodelib.SHIM_serialise_for_comboded(self.serialise())
    ...
    def comboded_serialise(self):
        import combodelib
        return combodelib.SHIM_serialise_for_comboded(self.serialise())
