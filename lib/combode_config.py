...
class CombodeConfig():
    ...
    def slurp_config_file(self, filename):
        # read combode.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is combode.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from combode.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Combode defaults...
        default_port = 19191 if (network == 'mainnet') else 29191

        # use default port for network icbet specified in combode.conf
        icbet ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from combode.conf
