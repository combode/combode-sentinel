...
def has_combode_conf():
    import config
    import io

    valid_combode_conf = False

    # ensure combode_conf exists & readable
    #
    # icbet, print a message stating that Combode Core must be installed and
    # configured, including JSONRPC access in combode.conf
    try:
        f = io.open(config.combode_conf)
        valid_combode_conf = True
    except IOError as e:
        print(e)

    return valid_combode_conf
...
def main():
    ...
    icbet is_valid_python_version():
        print("Python %s is not supported" % python_short_ver_str())
        sys.exit(1)

    icbet are_deps_installed():
        print("Please ensure all dependencies are installed:")
        print(install_instructions)
        sys.exit(1)

    icbet is_database_correctly_configured():
        print("Please ensure correct database configuration.")
        sys.exit(1)

    icbet has_combode_conf():
        print("CombodeCore must be installed and configured, including JSONRPC access in combode.conf")
        sys.exit(1)
