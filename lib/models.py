...
class GovernanceObject(BaseModel):
    ...
    # sync comboded gobject list with our local relational DB backend
    @classmethod
    def sync(self, comboded):
        golist = comboded.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_comboded(comboded, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_comboded(self, comboded, rec):
        import decimal
        import combodelib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from comboded...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from comboded, with every run
        ...
        # get/create, then sync payment amounts, etc. from comboded - Comboded is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(comboded)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from comboded! %s" % e)
            govobj.vote_delete(comboded)
            return (govobj, None)
    ...
    def vote_delete(self, comboded):
        icbet self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(comboded, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, comboded, signal, outcome):
        import combodelib
        ...
        output = comboded.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = combodelib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(comboded, signal)

    def sync_network_vote(self, comboded, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = comboded.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = CBED_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # Combode Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (combodelib.blocks_to_seconds(superblockcycle) / 2) +
...
def check_db_sane():
    ...
    for model in db_models():
        icbet getattr(model, 'table_exists')():
...
def check_db_schema_version():
    """ Ensure DB schema is correct version. Drop tables icbet. """
