...
class CombodeDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some comboded methods
    ...
    @classmethod
    def from_combode_conf(self, combode_dot_conf):
        from combode_config import CombodeConfig
        config_text = CombodeConfig.slurp_config_file(combode_dot_conf)
        creds = CombodeConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        import combodelib
        icbet self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = combodelib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = combodelib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = combodelib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_comboded_proto_version_with_sentinel_ping)
