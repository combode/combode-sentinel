...
def is_valid_combode_address(address, network='mainnet'):
    ...
    combode_version = 36 if network == 'testnet' else 27
    ...
    try:
        ...
        # rescue from exception, not a valid Combode address
        return False

    if (address_version != combode_version):
        return False
...
def SHIM_serialise_for_comboded(sentinel_hex):
    ...
    # superblock => "trigger" in comboded
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # comboded expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    comboded_hex = serialise(obj)
    return comboded_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('combode.conf')
