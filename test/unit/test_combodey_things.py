...
def mn_list():
    ...
    masternodelist_full = {
        u'701854b26809343704ab31d1c45abc08f9f83c5c2bd503a9d5716ef3c0cda857-1': u'  ENABLED 70201 yjaFS6dudxUTxYPTDB9BYd1Nv4vMJXm3vK 1474157572    82842 1474152618  71111 52.90.74.124:29190',
        u'f68a2e5d64f4a9be7ff8d0fbd9059dcd3ce98ad7a19a9260d1d6709127ffac56-1': u'  ENABLED 70201 yUuAsYCnG5XrjgsGvRwcDqPhgLUnzNfe8L 1474157732  1590425 1474155175  71122 [2604:a880:800:a1::9b:0]:29190',
        u'656695ed867e193490261bea74783f0a39329ff634a10a9fb6f131807eeca744-1': u'  ENABLED 70201 yepN97UoBLoP2hzWnwWGRVTcWtw1niKwcB 1474157704   824622 1474152571  71110 178.62.203.249:29190',
    }
...
def mn_status_good():
    ...
    status = {
        ...
        "service": "[2604:a880:800:a1::9b:0]:29190",
...
def test_valid_combode_address():
    from combodelib import is_valid_combode_address

    main = valid_combode_address()
    test = valid_combode_address('testnet')

    assert is_valid_combode_address(main) is True
    assert is_valid_combode_address(main, 'mainnet') is True
    assert is_valid_combode_address(main, 'testnet') is False

    assert is_valid_combode_address(test) is False
    assert is_valid_combode_address(test, 'mainnet') is False
    assert is_valid_combode_address(test, 'testnet') is True


def test_invalid_combode_address():
    from combodelib import is_valid_combode_address

    main = invalid_combode_address()
    test = invalid_combode_address('testnet')

    assert is_valid_combode_address(main) is False
    assert is_valid_combode_address(main, 'mainnet') is False
    assert is_valid_combode_address(main, 'testnet') is False

    assert is_valid_combode_address(test) is False
    assert is_valid_combode_address(test, 'mainnet') is False
    assert is_valid_combode_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from combodelib import elect_mn
...
def test_parse_masternode_status_vin():
    from combodelib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % combodelib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(combodelib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(combodelib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(combodelib.blocks_to_seconds(16616)) == 2612035
